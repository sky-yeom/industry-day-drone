...
GREETING_SECURITY = (
    "안녕! 난 Gibby라고해! 지금 " + "·".join(_SECURITY_SITE_NAMES) + ", 이렇게 세 곳에서 동시에 경보가 울렸어. "
    "드론은 한 대뿐이라 한 곳씩만 확인할 수 있는데, 그중 두 곳은 오경보고 한 곳에만 진짜 침입자가 있어. "
    "먼저 침입자가 어떤 모습인지 말해줄 수 있어?"
)
_CONSTRUCTION_SITE_NAMES = [p["siteName"] for p in CONSTRUCTION_SCENARIO["people"]]
GREETING_CONSTRUCTION = (
    "안녕! 난 Gibby라고해! 지금 건설 현장 " + "·".join(_CONSTRUCTION_SITE_NAMES) + ", "
    "이렇게 세 구역을 드론 한 대로 안전 점검해야 해. 핑크색 작업복을 입고 안전모를 쓰지 않은 사람을 찾아서 "
    "안전관리자에게 신고할 거야. 먼저 어떤 사람을 찾아야 하는지 말해줄 수 있어?"
)
