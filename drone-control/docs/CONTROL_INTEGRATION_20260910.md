...
| 구성 | 역할 | 이번 변경 |
|---|---|---|
| 기존 대시보드 | 음성 입력·선택 순서·지도 애니메이션·이미지·결과 표시 | 9월 13일 main `0c06be0`의 최신 팀원 UI 유지 |
...
9월 13일 main의 지도·드론 탑승 애니메이션과 Voice 대화/확인 문구는 수정하지 않습니다.
`route_intro.ready` 이전에는 기존처럼 다음 질문을 보류합니다.
웹과 relay는 함께 갱신해 `after-playback-v1` 음성 입력 창 프로토콜을 맞춥니다.
정상적인 "불난 집이요", "불난 집으로 가고 싶어", "출발해 주세요"도 인식하되,
부정·복수 목적지·조건부 출발 문장을 동의로 바꾸거나 새 참가자 턴 검사를 생략하지 않습니다.
상태 API의 `bridgeConnected`는 PC↔폰 통신, `physicalConnected`는 DJI 기체 연결입니다.
기체 상태가 없으면 `null`이며, 폰 TCP 연결만으로 실제 기체 연결을 주장하지 않습니다.
...
단독 PC 캡처 mock은 RGB/RGBA 원본과 픽셀이 다른 `SIMULATED 2` 버전을 반환하며,
미리보기는 같은 원본을 960픽셀·512KB 한도 안으로 축소합니다.
원본 현장 파일을 덮어쓰거나 실기 영상의 한도를 늘리지는 않습니다.
시한 만료·작전 중단으로 분석이 취소된 이미지는 `captured`/`evidence:null`로 남습니다.
이미 끝난 작전을 `analyzing`으로 표시하거나 미확인 결과를 만들어내지 않습니다.
독립 Node mock과 내장 Python mock의 PNG 압축 방식이 달라도, 같은 생성 프레임인지
크기와 전체 픽셀 바이트로 비교합니다. 파일의 표식만으로 가짜 성공을 허용하지 않습니다.
