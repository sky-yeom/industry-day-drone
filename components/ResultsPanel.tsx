...
export default function ResultsPanel({ state, debrief, onReset }: {
  ...
  return <section className="flex h-full min-h-0 w-full flex-col gap-3 p-3 sm:p-4">
    <div className="flex shrink-0 items-center justify-between gap-3">
      <div>
      <p className="text-[10px] font-bold tracking-[0.2em] text-white [text-shadow:2px_2px_0_#091f2c]">작전 최종 설명</p>
      <h2 className="text-lg font-semibold text-white [text-shadow:2px_2px_0_#091f2c]">결과</h2>
      </div>
    ...
    {!terminal ? <div className="pixel-panel flex min-h-0 flex-1 items-center justify-center bg-white p-4 text-center text-sm text-[#8c8279]">
      아직 구조 결과가 없습니다. 작전 종료 후 여기에 표시됩니다.
    </div> : <>
      <div className="pixel-panel shrink-0 bg-[#463668] px-4 py-3 text-center text-white">
      ...
      <div className="grid shrink-0 grid-cols-3 gap-2">
        {state.people.map((person) => <article key={person.id} className="pixel-panel p-2">
          <h3 className="text-xs font-semibold text-[#463668]">{MONITOR_MAP[person.monitorId].label}</h3>
