...
LABELS = {mid: f"현장 {i}" for i, mid in enumerate(MONITOR_IDS, 1)}
...
class SurveySession:
    ...
    def confirm_prompt(self, prompt_text, appearance_constraints=None, unsupported_appearance=None):
        ...
        return result(True, f"사용자가 확인한 탐색 프롬프트: {text}. 현장별 신고 내용: {cases}",
                      "먼저 세 현장의 신고 내용을 모두 설명한 뒤 첫 번째로 갈 현장을 물어볼 것")
    ...
    def select_stop(self, monitor):
        ...
        if not isinstance(monitor, str) or monitor not in MONITOR_IDS:
            return result(False, "현장을 확인하지 못했습니다.", "현장 1, 2, 3 중 어디인지 다시 물어볼 것")
        if monitor in self.state.draftRoute:
            return result(False, "이미 경로에 있는 현장입니다.", "수정하려면 경로를 지울지 물어볼 것")
    ...
    def debrief(self):
        ...
        for person in self.data["people"]:
            ...
            observations.append(f"현장 {person['monitorId'][-1]}: {detail}")
