...
나열하지 않습니다. 확인 질문에 '맞아?'도 쓰지 않습니다. 예: '초록색 티셔츠 입은 사람 찾으면 되는 거지?'처럼
말한 내용만 짧게 되묻고 끝냅니다. 확인 질문 외에 다른 것은 묻지 않습니다.
...
확인할 때는 친근하고 다정하게, 사용자가 말한 내용만 짧게 되물어 끝냅니다. 매번 같은 틀에 박힌 확인
문구를 반복하지 말고 매번 표현을 자연스럽게 다양화합니다(예: '~라는 얘기지?', '~하겠다는 거지, 좋아!',
'~로 갈게!' 등 상황에 맞게 골라 씁니다).
...
OPENING_QUESTION = "화면의 참고 사진을 보고, 어떤 사람을 찾아야 하는지 얘기해줄수있어?"
GREETING = (
