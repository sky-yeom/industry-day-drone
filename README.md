# 긴급 신고 정찰 작전 — Emergency Triage

Industry Day 데모: 세 사람의 위급함을 판단하고 드론의 방문 순서를 정하는
한국어 드론 신고 체험입니다. 드론은 사람을 직접 구조하지 않고, 대상자를 찾아
정확한 위치와 상태를 119 종합상황실에 신고하는 정찰·신고 역할입니다. 실제
구조는 신고를 받은 119가 출동해서 수행합니다. Azure AI Voice Live의 실시간
음성 대화로 참여하며 프롬프트 작성·확인, 경로 선택, 출발은 음성으로 조작합니다.
네 탭은 클릭해서 자유롭게 살펴볼 수 있으며, 출발 후 드론은 자동으로 이동·촬영·분석합니다.

**가상의 훈련 시나리오입니다.** 이미지에서 대상자를 찾아 위치를 신고하는 것을
데모 규칙으로 다루며, 실제 신고·구조 활동이나 의료 판단을 대신하지 않습니다.
...
| 모니터 2 | 잔해 아래에 갇혀 가벼운 부상을 입은 사람 | 세 경우 중 상대적으로 낮음, 신고 시한은 있음 |
| 모니터 3 | 불이 난 집에 갇힌 사람 | 가장 높음 |

Gibby는 출발 전에 두 가지 추천 순서를 제안합니다: 현재 위급도만 본 순서와,
노약자·취약 대상자를 고려해 한 단계 앞당긴 순서입니다. **최종 순서는 항상
참가자가 직접 선택**하며, 이 게임의 기본 취약 대상은 모니터 2(잔해 아래)입니다.
두 추천 모두 앞당겨진 조합이 아닌 이상, 게임의 기준 순서는 **불길 → 바다 →
잔해 (3 → 1 → 2)**입니다. 이는 가상 시나리오의 규칙이며 일반적인 의료
우선순위나 실제 생존 시간을 뜻하지 않습니다. 참가자가 다른 순서를 고르더라도
시스템이 몰래 고치지 않습니다. 첫 번째와 두 번째 지점을 정하면 남은 곳을 마지막에
추가하고, **출발에 동의한 순간부터 실제 시간이 흐릅니다.**

도착만으로는 신고되지 않습니다. 해당 지점의 **촬영 이미지에서 대상자를 확인한
분석 결과가 신고 시한 전에 도착해야** 119 신고가 인정됩니다. 이동·촬영·분석·재촬영
중에도 시간이 흐르며, 촬영이 빨라도 분석 완료가 늦으면 신고 시한을 놓칩니다.
...
Azure 분석은 같은 후보에게 구조가 필요하다는 시각적 근거와, 그 판단에 대한
확신도(0~100 정수)도 함께 반환합니다. 확신도는 판정 가능 여부나 구조 필요
여부와는 별개로, 시각적 근거가 뚜렷한 정도만 나타냅니다. 현장 신고는 맥락으로만
사용하며, 이미지 전체를 분석하고 박스 좌표는 생성하지 않습니다.

| 결과 | 판정 |
| --- | --- |
| 신고 완료 | 시한 전에 대상자를 확인했고 부상이 없는 경우 |
| 부상 확인 후 신고 | 시한 전에 확인했지만 원래 부상이 있거나 상태가 악화된 경우 |
| 신고 시한 초과 | 119에 신고할 수 있는 시한이 확인 없이 지난 경우 |

모니터 2는 처음부터 부상이 있으므로 제때 확인해도 **부상 확인 후 신고**로
기록됩니다. 두 가지 신고 성공 결과 모두 신고 인원에 포함됩니다.
...
### 예외 처리: 판정 불가·거부되는 설명 예시

아래 표는 참가자가 어떤 설명을 말했을 때 왜 거부되는지, 어떤 안내가 나오는지
보여 주는 대표 사례입니다. 어떤 경우에도 시스템이 설명을 임의로 고치거나,
빠진 특징을 추측해서 채우지 않습니다.

| 참가자 발화(예시) | 거부 사유 | 참가자에게 돌아오는 안내 |
| --- | --- | --- |
| "백인 남자를 찾아줘" | 인종·민족을 이미지에서 추론하는 조건은 애초에 받지 않음 | `relay/appearance.py`의 `validate_search_prompt`가 즉시 거부, 예시·힌트 없이 재설명 요청 |
| "이 사진이랑 똑같이 생긴 사람" | 동일인 식별(생체 인식)에 해당 | 위와 동일하게 신원 확인 관련 문구로 즉시 거부 |
| "안경 쓴 사람" (mock 모드) | Mock 모드의 고정 문법(색상·머리·상의)에 없는 특징 | `prepare_prompt`/`confirm_prompt`가 경로 선택 전에 거부. Azure 모드에서는 원문 그대로 전달되어 거부되지 않음 |
| "그냥 사람 아무나" | 조건 없이 사람만 지칭 — 탐지 조건이 되지 않음 | 재설명 요청, 경로 선택 불가 |
| 확인된 설명이 있으나 이미지가 너무 어둡거나 각도가 애매해 분석 모델이 판정 불가로 응답 | 모델이 `assessable=false`로 반환한 경우 | 확정된 조건은 그대로 유지한 채 임무가 즉시 중단(abort)됨 — 같은 조건으로 재시도하지 않고, **처음으로**에서 다시 설명·확인 필요 |
| "얼굴로 신원 확인해줘" | 안면 인식/식별 요청 | 신원 확인 관련 문구로 즉시 거부 |

이 표에 없는 새로운 거부 사례를 추가하려면 `relay/appearance.py`의
`validate_search_prompt`(민감 조건 정규식) 또는 `relay/vision.py`의
`assessable` 처리 분기를 먼저 확인하고, 해당 함수를 다루는
`relay/test_appearance.py`·`relay/test_vision.py`에 대응하는 케이스를
추가하세요.

실제 드론 연결은 추후 작업입니다. `relay/camera.py`의 캡처 어댑터를 실제
장치의 이미지 수신 방식에 연결하면 됩니다. 프레임은 지점·캡처 ID와 연결되어야
하며, 모니터 번호만 받거나 클라이언트가 보낸 성공 여부만 믿고 신고 처리하면
...
브리핑·대상자 묘사·초기 부상·신고 시한·모의 비행 시간·이미지가
이 파일의 단일 정의를 사용합니다.

`injuryWindowMs: 5000` is shared by the relay and frontend. A report confirmed
with five seconds or less remaining counts as **부상 확인 후 신고**, including
exactly five seconds remaining. A confirmation at the deadline is **신고 시한
초과 (report_missed)**. Initially injured people remain injured even when
reported earlier.

기본 모의 환경은 이동 7초, 촬영 1초, 분석 2초로 설정되어 있습니다.
게임상 신고 시한은 모니터 3이 18초, 모니터 1이 28초, 모니터 2가 45초입니다.
바다의 대상자는 23초부터 상태가 악화되어 제때 탐지해도 부상 상태로 집계됩니다.
따라서 정상 모의 탐지에서 3 → 1 → 2는 세 사람을 모두 제때 신고할 수 있고,
...
> 안녕! 난 Gibby야! 너는 119종합상황실 소속 상황요원이고, 방금 익명 문자로 사진이랑 같이 위급 신고가 들어왔어. 바다, 잔해 아래, 불이 난 집, 이렇게 세 곳에 사람이 있는데 드론은 한 대뿐이라 한 곳씩 차례로 찾아서 위치를 신고해야 해. 네가 오더만 내려주면 내가 드론 보낼게! 먼저 바다에서 어떤 사람을 찾아줘야 하는지 말해줄 수 있어?

첫 인사의 요약·의역이나 추가 안내는 허용하지 않습니다. 응답별 안내를
추가할 때에도 세션의 대화 규칙을 유지하며, 에이전트가 참가자의 프롬프트를
대신 만들거나 다른 외형을 지어내지 않도록 합니다.

출발이 확정되면 **“출발한다! 경로 따라서 탐색하고 119에 신고 시작할게!”**를
말합니다.
...
| 무엇 | 파일 |
| --- | --- |
| 공유 시나리오·신고 시한·이미지 | `data/emergency-triage.json` |
| 경로·시계·신고 판정 | `relay/survey.py` |
