...
class LiveAdapter:
    ...
    def __init__(self, site_path, config_path):
        ...
        if (site["physical_leftward_tag_ids"] != [2, 1, 3] or site["home_tag_id"] != 2
                or site["floor_tag_id"] != 0 or site["target_height_m"] != 1.4
                or site["expected_bridge_build_id"] != BUILD_ID):
            raise ValueError("Live adapter supports the established ID2/1/3 corridor and 1.4m profile")
        ...
        if not self.config.actual_measurements_confirmed or self.config.patrol is None or not {0, 1, 2, 3} <= set(self.config.tag_map):
            raise ValueError("Measured camera/tag configuration is required")
        ipaddress.ip_address(self.config.network.host)
        if not self.config.network.confirmation_token or self.config.network.confirmation_token == "REPLACE_ME":
        ...
        self.config = replace(self.config, network=replace(self.config.network, rate_hz=10),
            patrol=replace(self.config.patrol, obstacle_stop_m=0., cruise_altitude_m=1.4,
    ...
    def run(self, mission, cancel, emit):
        from ..vision import TcpVideoStream, AprilTagDetector
        ...
        try:
            ...
            stream = FreshVideoStream(self.config.network.host, self.config.network.video_port, self.config.network.video_codec)
            ...
            while time.perf_counter() < deadline:
                ...
                if age <= .5 and any(t.tag_id == 0 for t in tags):
                    break
            else:
            ...
            home = VisitGate(2, PatrolPhase.WALL_HOME)
            ...
            for visit in mission["visits"]:
                ...
                if previous == expected:
                    ...
                    direction = "left" if self.site["physical_leftward_tag_ids"].index(expected) > self.site["physical_leftward_tag_ids"].index(previous) else "right"
                    _traverse_to_expected(client, limiter, stream, detector, logger, gate, self.config.patrol,
                        direction=direction, target_x=self.config.camera.cx, target_y=self.config.patrol.wall_target_y_px,
            ...
            if previous != 2:
                gate = VisitGate(2, PatrolPhase.RETURN)
                _traverse_to_expected(client, limiter, stream, detector, logger, gate, self.config.patrol,
                    direction="right", target_x=self.config.camera.cx, target_y=self.config.patrol.wall_target_y_px,
                    departure_tag_id=previous)
            client.zero()
            completed = True
        except Exception as exc:
    ...
    def _climb(self, client, limiter, stream, detector, visual_height):
        ...
        raise RuntimeError("1.4m ascent target unconfirmed after 8s")
    ...
    def _capture(self, client, limiter, stream, detector, tag, index, emit):
        ...
        while time.perf_counter() < deadline:
            ...
            if not stable or age > .5 or not any(d.tag_id == tag for d in tags):
                held = None
                continue
