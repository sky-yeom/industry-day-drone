...
export default function DroneImagePanel({ captures }: { captures: CapturedImage[] }) {
  ...
  return <section className="flex h-full min-h-0 flex-col gap-2 p-3">
    <div className="flex shrink-0 items-center justify-between gap-3">
      <div>
        <p className="text-[10px] font-bold tracking-[0.2em] text-[#8661c5]">카메라 이미지 수신</p>
        <h2 className="text-lg font-semibold text-[#091f2c]">드론 이미지</h2>
      </div>
      <span className="shrink-0 rounded-full bg-[#fff8f3] px-3 py-1 text-xs font-semibold text-[#7f5a1a]">
    ...
    {current && <nav className="flex shrink-0 items-center justify-between gap-2 text-xs text-[#8661c5]" aria-label="촬영 기록">
      <button type="button" disabled={index <= 0} onClick={() => setSelectedId(captures[index - 1].id)} className="rounded-lg border px-3 py-1 disabled:opacity-30">이전 촬영</button>
      <span className="tabular-nums">{index + 1} / {captures.length}</span>
