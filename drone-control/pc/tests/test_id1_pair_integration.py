...
class PairDispatchTests(StandaloneTestCase):
    ...
    def test_field_flight_flag_tolerance_reaches_the_actual_dispatch_path(self):
        for age, allowed in ((517., True), (1500., True), (1501., False)):
            clock = [100.]
            with self.subTest(age=age), \
                    patch.object(shuttle.time, "monotonic", lambda: clock[0]), \
                    patch.object(shuttle.time, "perf_counter", lambda: clock[0]):
                client, wire = self.client(clock, airborne_raw(is_flying_age_ms=age))
                if allowed:
    ...
    def test_field_flight_flag_tolerance_does_not_relax_velocity_or_height(self):
        for field in ("velocity_age_ms", "height_age_ms", "attitude_age_ms"):
            clock = [100.]
            with self.subTest(field=field), \
                    patch.object(shuttle.time, "monotonic", lambda: clock[0]), \
                    patch.object(shuttle.time, "perf_counter", lambda: clock[0]):
                client, wire = self.client(clock, airborne_raw(is_flying_age_ms=517., **{field:501.}))
                with self.assertRaises(InterruptedError):
                    client.attitude(0., .3, 0., 0.)
