...
3번째 -> 확정(연쇄 호출) -> 임무 브리핑 -> 주의사항 되묻기 확인 -> (임시)
탐지 -> 채점.

모니터 선택 단계는 되묻기/재확인 없이 진짜 답을 들으면 바로 도구가 불린다
("네/어" 같은 별도 동의를 더 기다리지 않는다). 첫 줄은 회귀 테스트: 모니터
얘기가 전혀 아닌 잡담(그 안에 "하나", "네" 같은 낱말이 우연히 섞여 있음)을
보내고, 그게 절대 monitor를 고른 것으로 처리되지 않는지 확인한다.

두 번째 경유지를 고르면 세 번째가 자동으로 채워지므로 같은 턴 안에서
select_stop에 이어 confirm_route까지 연쇄 호출돼야 한다.

경로 확정 이후의 주의사항 단계는 자유 문장이라 여전히 되묻기 확인을 거친다:
사용자가 주의사항을 말해도(아직 확인 전) 도구가 불리지 않아야 하고, "네"로
확인해야 confirm_prompt가 불린다. confirm_prompt가 성공하면 프롬프트에 따라
모델이 같은 턴 안에서 바로 report_detection까지 연쇄 호출해야 하므로, 이 줄은
도구 호출이 있어야 한다고 표시돼 있다(정확히 몇 개가 불렸는지는 확인하지
않는다).
