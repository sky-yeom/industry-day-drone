...
from .wall_framing import WallViewAction, wall_view_action, wall_view_bounds
...
def _traverse_to_expected(
    ...
    *,
    ...
    print(f"Moving {direction}; accepting ID {expected} inside broad view {wall_view_bounds(patrol)}")
    while time.monotonic() < deadline:
        limiter.wait()
        telemetry = client.last_telemetry
        _check_control(telemetry)
        ...
        view_action = None if observed is None else wall_view_action(
            observed.center_px, frame_shape, direction, patrol)
        ...
        if gate_action is not previous_gate_action:
            client.log_event(
                "patrol_tag_gate",
                {
                    ...
                    "framing_mode": "broad_view",
                    "view_bounds_fraction": wall_view_bounds(patrol),
                },
            )
            if (
        ...
        if observed is not None and view_action is not WallViewAction.INSIDE:
            ...
            payload["framing_mode"] = "broad_view"
            payload["view_bounds_fraction"] = wall_view_bounds(patrol)
...
def _pause_for_tag_photo(client, limiter, capture, detector, logger, phase,
    ...
    No image-centering or reverse correction. Timing alone is not evidence
    of a stationary aircraft. The observation recorder also keeps1Hz frames.
    """
    ...
    while time.monotonic() < deadline:
        ...
        inside = fresh and tag is not None and snapshot is not None and wall_view_action(
            tag.center_px,snapshot.frame.shape,direction,patrol) is WallViewAction.INSIDE
