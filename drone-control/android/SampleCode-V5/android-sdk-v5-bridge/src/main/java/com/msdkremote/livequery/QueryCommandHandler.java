...
public class QueryCommandHandler implements CommandHandler
{
    ...
    public void onCommand(@NonNull CommandServer commandServer, @NonNull String command)
    {
        ...
        boolean mutation = commandMethod.equalsIgnoreCase(COMMAND_SET)
                || commandMethod.equalsIgnoreCase(COMMAND_ACTION);
        if (mutation) {
    ...
    @NonNull
    private static String firstWord(@NonNull String value) {
        int split = value.indexOf(' ');
        return split < 0 ? value : value.substring(0, split);
    }

    @NonNull
    private static String remainingWords(@NonNull String value) {
        int split = value.indexOf(' ');
        return split < 0 ? "" : value.substring(split + 1).trim();
    }
