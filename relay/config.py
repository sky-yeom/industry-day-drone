...
# 곳에서는 잡음이 발화로 잡혀서 없는 말이 전사된다. 0.6이 데모 환경 기준점이다.
VAD_THRESHOLD = float(os.getenv("VOICE_LIVE_VAD_THRESHOLD", "0.6"))
...
# 140ms면 짧은 잡음은 걸러지면서 "네", "1" 같은 짧은 대답은 그대로 통과한다.
# 이 값은 응답 지연과는 무관하다. 발화 시작 판정에만 쓰인다.
SPEECH_DURATION_MS = int(os.getenv("VOICE_LIVE_SPEECH_DURATION_MS", "140"))
...
# 경우가 줄고, 잡음을 엉뚱한 단어로 채우는 것도 억제된다.
