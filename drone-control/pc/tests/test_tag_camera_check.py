...
class InspectionDetectorTests(TestCase):
    ...
    def make_detector(self, calibrated, *, home_configured=True):
        config = load_config(CONFIG)
        if not home_configured:
            config = replace(config, tags=tuple(tag for tag in config.tags if tag.id != 6))
    ...
    def test_unknown_ids_keep_quality_and_roles_without_fabricated_pose(self):
        detector, base = self.make_detector(True, home_configured=False)
    ...
    def test_configured_home_without_detected_pose_is_not_reported_as_unknown_size(self):
        detector, _ = self.make_detector(True)
        home = detector.detect(SimpleNamespace(ndim=2))[1]
        self.assertEqual(home["tag_id"], 6)
        self.assertTrue(home["configured"])
        self.assertEqual(home["pose_status"], "unavailable")
        self.assertIsNone(home["camera_to_tag_transform"])
