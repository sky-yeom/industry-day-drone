...
if (-not $ProfilePath) { $ProfilePath = Join-Path $PSScriptRoot 'profiles/standalone_tag_216.json' }
...
if ($Mode -eq 'Camera') {
    ...
    if ($Mode -eq 'Check') {
        ...
        if ($Mode -eq 'Patrol') {
            ...
            Write-Host 'PATROL: takeoff -> configured sonar height -> 6 -> ID1, ID2 mock photos -> 1,6 -> RC landing.'
        } elseif ($Mode -eq 'ID1') {
            $runnerArgs += '--id1-pair'
            Write-Host 'ID1: takeoff -> configured sonar height -> wall6 -> ID1 + full mock photo -> hover -> RC handover.'
        } else {
            Write-Host 'FLIGHT: takeoff -> configured sonar height -> wall6 -> left 1,2 -> right 1,6 -> RC landing.'
        }
    } else {
